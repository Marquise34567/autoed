        ...
        return (
          <div key={s.key} data-step={s.key} className="flex items-center gap-4">
            <div className={`flex flex-col items-center gap-2 ${i === 0 ? '' : ''}`}>
              <div className={`h-14 w-14 rounded-full flex items-center justify-center text-sm font-semibold transition-all transform ${active ? 'bg-linear-to-br from-[#7c3aed] to-[#06b6d4] shadow-[0_8px_30px_rgba(124,58,237,0.28)] scale-105 text-white ring-4 ring-[rgba(124,58,237,0.12)]' : done ? 'bg-white/6 text-white/90' : 'bg-[rgba(255,255,255,0.02)] text-white/50 border border-white/6'}`}>
                {done ? (
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-emerald-300" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414L9 14.414l-3.707-3.707a1 1 0 10-1.414 1.414l4.414 4.414a1 1 0 001.414 0l8.414-8.414a1 1 0 00-1.414-1.414z" clipRule="evenodd"/></svg>
                ) : (
                  <div className="text-sm">{i+1}</div>
                )}
              </div>
              <div className="text-left">
                <div className={`text-sm ${active ? 'text-white' : 'text-white/70'} font-semibold`}>{s.label}</div>
                <div className="text-xs text-white/50">{s.subtitle}</div>
              </div>
            </div>
            {i < steps.length-1 && <div className={`h-1 flex-1 ${done ? 'bg-linear-to-r from-emerald-400 to-cyan-400' : 'bg-white/6'} mt-7 rounded-full`} />}
          </div>
        )
      })}
    </div>
