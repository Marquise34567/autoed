...
export default function CenterPreviewPanel({ children, status, overallProgress }:{ children?:React.ReactNode; status?:any; overallProgress?:number }){
  ...
  return (
    <div className="col-span-1 lg:col-span-6 flex flex-col gap-4">
      <div className="relative w-full rounded-3xl bg-[rgba(255,255,255,0.02)] border border-white/6 p-4 backdrop-blur-md shadow-inner overflow-hidden">
        <div className="w-full h-96 bg-black/80 rounded-2xl overflow-hidden flex items-center justify-center">
          {children}
        </div>

        {/* Status card top-right */}
        <div className="absolute right-4 top-4 bg-[rgba(0,0,0,0.45)] border border-white/6 rounded-xl p-2 w-48">
        ...
        <div className="mt-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button className="p-2 rounded-full bg-white/6 text-white/90">Play</button>
            <div className="text-xs text-white/60">00:00 / 00:00</div>
          </div>
